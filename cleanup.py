...
for y in range(image_data.shape[0] ): #2800,3600
    for x in range(image_data.shape[1]): #1000,1800
...
image_data = np.delete(image_data, np.s_[0:75], 0)
image_data = np.delete(image_data, np.s_[-75:], 0)
image_data = np.delete(image_data, np.s_[0:75], 1)
image_data = np.delete(image_data, np.s_[-75:], 1)
...
def histogram(data):
    ...
    plt.plot(x, gaussian(mu,std,x), 'k', label = 'Gaussian fit: $\mu$ = 3419.1, $\sigma$ = 12.7')
