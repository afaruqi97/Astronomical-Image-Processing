...
def make_plot(magnitudes,mag_errs):
    ...
    mags = np.linspace(min(magnitudes) + 0.5 ,max(magnitudes) + 3,50) # magnitude bins
    for m in mags:
    ...
    grad = (logNs[20] - logNs[9])/(mags[20] - mags[9])
    #upper_grad = (logNs[5] - logNs[0])/(mags[5] - mags[0])
    print(grad)
    #print(upper_grad)
    plt.plot(mags[8:22], [(grad*mag) - 1.7 for mag in mags[8:22]], 'g--')
    #plt.plot(mags[0:7], [(upper_grad*mag) - 4.2 for mag in mags[0:7]], 'b--')
    plt.xlabel('m')
    plt.ylabel('$log_{10}$N(<m)')
    #plt.savefig('NvsM.jpg')
    plt.show()
